...
Available now
File	Feature	Needs
`movement/noclip.lua`	Walk through walls	—
`movement/flight.lua`	Fly with WASD	—
`movement/walkspeed-and-slide.lua`	Faster running and sliding	`debug.getupvalues`, `debug.setupvalue`
`movement/jump-power.lua`	Jump higher	`debug.getupvalues`, `debug.setupvalue`, `debug.getconstants`, `debug.getprotos`, `debug.getproto`
`movement/auto-strafe.lua`	Gain air speed while holding jump	—
`movement/long-jump.lua`	Launch yourself across the map	—
`movement/movement-recorder.lua`	Record a route once, then have the game walk it for you	`writefile`/`readfile` (only to save between sessions)
`movement/animation-player.lua`	Loop any Roblox animation on your character	—
`automation/auto-pickup.lua`	Grab health when hurt, ammo when empty	`firetouchinterest`
`automation/auto-respawn.lua`	Respawn the moment the game allows it	—
`automation/tripmine-auto-trigger.lua`	Set off Subspace tripmines from across the map	`firetouchinterest`
`automation/auto-ban-and-map-vote.lua`	Ban weapons and vote for maps from a list you write once	—
`automation/auto-loadout.lua`	Fill in and confirm the weapon picker for you, per map	—
`automation/auto-queue.lua`	Requeue the instant a match ends, skipping the cutscene	`VirtualInputManager` (only for Skip)
`combat/weapon-mods.lua`	Twelve weapon and camera modifiers: attack speed, reload, recoil, dash cooldown, ADS, equip, automatic fire, infinite double jumps, camera shake, third person, FOV, viewmodel offset	—
`combat/no-spread-and-grenade-fuse.lua`	Bullets with no scatter; choose when throwables detonate	`setrawmetatable`, `clonefunction`, `debug.getupvalues`, `debug.setupvalue`
`combat/aimbot.lua`	Pulls your aim onto a target while you hold a key	`Drawing` (only for the FOV circle)
`combat/flickbot.lua`	Snaps to a target along a human-shaped path, fires, and stops	`VirtualInputManager` (only to shoot)
`combat/triggerbot.lua`	Fires for you when your crosshair is already on someone	—
`combat/ragebot.lua`	Stops aiming entirely — moves what the server believes about you onto the target and fires directly. By far the most detectable thing here.	`sethiddenproperty`, `setthreadidentity`
`crosshair/custom-crosshair.lua`	Your own crosshair: shape, colours, spin, breathing gap, text	`Drawing`
`esp/player-esp.lua`	Boxes, names, health, distance, look lines and off-screen arrows through walls	`Drawing`
`esp/throwable-esp.lua`	Labels on live grenades, molotovs, satchels and tripmines	`Drawing`
`visuals/bullet-tracers.lua`	A beam along every shot fired in the match, yours and theirs	—
`visuals/hit-and-kill-feedback.lua`	A sound and a body flash when you hit or kill someone	—
`visuals/model-chams-and-highlight.lua`	Recolour your character, arms and gun; outline the arms and gun	—
`visuals/sound-visualizer.lua`	A ring in the world wherever a sound plays — ESP with no Drawing API	—
`visuals/overlay-removals.lua`	Delete the vignette, tracers, muzzle flash and scope overlays	`debug.setconstant`, `debug.setupvalue` (partial without)
`visuals/stretched-resolution.lua`	Squash the view so targets look wider	—
`visuals/viewmodel-animations.lua`	Strip the first-person sway, shoot, sprint, equip and reload animations	—
`world/lighting.lua`	Rewrite the map's lighting, fog, shadows and post-processing, locally	—
`world/lightning.lua`	Procedural lightning strikes with branches, sparks and delayed thunder	—
`world/weather.lua`	Snow, rain or a blizzard that follows you around the map	—
`world/skybox.lua`	Replace the map's sky with a preset or your own six faces	—
`world/ambience.lua`	A looping background sound over the map	—
`misc/device-spoof.lua`	Report a different input device to the server	`debug.getupvalues`, `debug.getupvalue`, `debug.setupvalue`
`misc/staff-detector.lua`	Warn when a RIVALS moderator — or a friend of one — is in the server	`game:HttpGet` (only for the friends half)
`misc/claim-all-rewards.lua`	Claim every daily, battle pass and capsule reward in one go	—
`misc/unlock-cosmetics.lua`	Every skin, wrap, charm, finisher and emote shown as owned and equippable — on your screen only	`debug.getconstants`, `debug.setconstant` (menu half works without)
`debug/silent-debug.lua`	A live panel naming whichever of eleven signals is currently stopping a shot	—
Coverage
...
`visuals/overlay-removals.lua` covers six of
...
crosshair wrapper.
`misc/unlock-cosmetics.lua` is the unlock itself
...
`KiciaHook_Source_Runnable.lua` has all of it, unabridged.
---
Reading order, if you're here to learn
Start with `crosshair/custom-crosshair.lua`
...
`movement/noclip.lua` — the simplest complete
...
`movement/long-jump.lua` — the lesson the other
...
`combat/weapon-mods.lua` — the two techniques
...
`combat/no-spread-and-grenade-fuse.lua`
...
`esp/player-esp.lua` — the only drawing-heavy file
...
`combat/aimbot.lua` — target scoring in screen space
...
`visuals/viewmodel-animations.lua` —
...
`visuals/bullet-tracers.lua` — the quietest
...
`visuals/overlay-removals.lua` — six
...
`visuals/sound-visualizer.lua` — the
...
`combat/ragebot.lua` — read it for the first
...
`debug/silent-debug.lua` — the shortest useful
...
an afternoon.
`misc/unlock-cosmetics.lua` — the last word
...
`misc/staff-detector.lua` — nothing clever
...
`movement/movement-recorder.lua` — the
...
`visuals/model-chams-and-highlight.lua`
...
`world/lightning.lua` — nothing to do with cheating
...
`automation/tripmine-auto-trigger.lua`
...
from that state.
`combat/flickbot.lua` — read after the aimbot. It
